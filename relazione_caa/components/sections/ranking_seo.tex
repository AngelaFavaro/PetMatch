...
\begin{itemize}
    \item \textbf{Microformati nel footer:} utilizzo di microformati per le informazioni di contatto. Sono stati inseriti selettori di attributo \texttt{class} riconosciuti dai motori di ricerca per evidenziare il contenuto in modo semanticamente corretto. Il tag \texttt{address} da solo non è sufficiente poiché non specifica la natura delle informazioni contenute. È stato anche aggiunto l'attributo \textit{rel="e"} per rendere evidente la pagina social;
...
Durante lo sviluppo del progetto abbiamo notato che le pagine che mostrano molte immagini risultavano molto pesanti da renderizzare. Abbiamo modificato la funzione che importa le immagini per comprimere le immagini arrivando ai 40KB prima di inserirle nel server. Le immagini perdono un po' di qualità ma per lo spazio ridotto che occupano non è un peggioramento evidente e ne risulta un miglioramento notevole nei tempi di caricamento delle pagine.\\
...
Un'altra tecnica utilizzata per migliorare i tempi di caricamento è stata l'implementazione del lazy loading per le immagini.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{assets/lighthouse-animali.png}
    \caption{Risultati Lighthouse per la pagina animali}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{assets/lighthouse-admin.png}
    \caption{Risultati Lighthouse per la pagina area personale}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{assets/lighthouse-home.png}
    \caption{Risultati Lighthouse per la pagina home}
\end{figure}
