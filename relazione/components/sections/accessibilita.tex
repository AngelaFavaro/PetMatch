...
\begin{itemize}
    ...
    \item L'ordine degli ultimi movimenti e delle richieste di adozione è in ordine di ultima modifica, non di creazione.
\end{itemize}
\subsection{Robusto}
...
\begin{itemize} 
    ...
    \item la validazione dei form di inserimento dati è garantita lato server in PHP. Il controllo tramite JavaScript è implementato per fornire un \textit{feedback} immediato all'utente, ma non sostituisce i controlli di sicurezza del server; 
    \item la navigazione del menù principale e la commutazione del tema (Light/Dark mode) rimangono funzionali anche senza script, essendo basate su elementi standard HTML e fogli di stile CSS; 
