...
\subsection{Design emozionale}
L'obiettivo di PetMatch non è solo fornire un servizio funzionale, ma instaurare un legame positivo con l'utente attraverso strategie di design emozionale, agendo sui tre livelli di elaborazione cognitiva: viscerale, comportamentale e riflessivo.

\begin{itemize}
    \item \textbf{Piacere:} L'estetica curata, caratterizzata da una palette di colori caldi e accoglienti, trasmette un senso di familiarità. Il piacere deriva anche dalla sicurezza psicologica: la sezione dedicata al procedimento di adozione guida l'utente in 5 semplici passi, riducendo l'ansia legata all'incertezza del processo. La presenza degli eventi, inoltre, avvicina l'utente al mondo reale del rifugio, umanizzando il servizio digitale.
    
    \item \textbf{Sorpresa e Micro-interazioni:} Per gratificare l'utente e rendere la navigazione memorabile, sono stati inseriti elementi dinamici inaspettati:
    \begin{itemize}
        \item Le \textbf{polaroid degli eventi} che oscillano al passaggio del mouse simulando il vento.
        \item La \textbf{timeline interattiva} che rivela GIF animate al passaggio del mouse per spiegare le fasi di adozione.
        \item Il \textbf{tasto "torna su"} a forma di gatto che si anima durante l'hover. 
    \end{itemize}

    \item \textbf{Esclusività:} Poiché l'adozione è un atto unico, la piattaforma utilizza etichette come "animale già interessato" per informare l'utente che sono già presenti altre richieste. Questo, oltre a garantire trasparenza, genera un senso di urgenza e importanza che motiva l'utente a non procrastinare la propria domanda di adozione.

    \item \textbf{Anticipazione:} Il design sfrutta la curiosità dell'utente per guidarlo verso l'approfondimento delle informazioni:
    \begin{itemize}
        \item Nella \textit{home page}, le polaroid mostrano solo gli ultimi eventi, agendo da anticipazione per la sezione completa.
        \item Le \textbf{card degli animali} presentano solo informazioni generali (nome, età, taglia); questo "gap informativo" crea anticipazione, spingendo l'utente a cliccare per scoprire la storia completa dell'animale nella scheda di dettaglio.
    \end{itemize}

    \item \textbf{Reward:} Il percorso culmina nel raggiungimento del "match perfetto". Il completamento della richiesta di adozione rappresenta la gratificazione finale per l'utente, che trasforma l'interazione digitale nella possibilità concreta di accogliere un nuovo compagno animale.
\end{itemize}